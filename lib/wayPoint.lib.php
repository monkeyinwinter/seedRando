...
function wayPointAdminPrepareHead()
{
    ...
    //	'entity:-tabname:Title:@seedrando:/seedrando/mypage.php?id=__ID__'
    //); // to remove a tab
    complete_head_from_modules($conf, $langs, $object, $head, $h, 'wayPoint');
...
function wayPoint_prepare_head(wayPoint $object)
{
    ...
    $h = 0;
    $head = array();
    $head[$h][0] = dol_buildpath('/seedrando/card_wayPoint.php', 1).'?id='.$object->id;
    $head[$h][1] = $langs->trans("seedrandoCardWayPoint");
    $head[$h][2] = 'card';
    $h++;
...
function getFormConfirmseedrando(&$PDOdb, &$form, &$object, $action)
{
    ...
    {
        $text = $langs->trans('ConfirmValidatewayPoint', $object->ref);
        $formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?id=' . $object->id, $langs->trans('ValidatewayPoint'), $text, 'confirm_validate', '', 0, 1);
    }
    elseif ($action == 'delete' && !empty($user->rights->seedrando->write))
    {
        $text = $langs->trans('ConfirmDeletewayPoint');
        $formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?id=' . $object->id, $langs->trans('DeletewayPoint'), $text, 'confirm_delete', '', 0, 1);
    }
    elseif ($action == 'clone' && !empty($user->rights->seedrando->write))
    {
        $text = $langs->trans('ConfirmClonewayPoint', $object->ref);
        $formconfirm = $form->formconfirm($_SERVER['PHP_SELF'] . '?id=' . $object->id, $langs->trans('ClonewayPoint'), $text, 'confirm_clone', '', 0, 1);
    }
